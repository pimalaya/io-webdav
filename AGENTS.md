...
Check the structure yourself against the strict rules (CAIRN.md section 8): a discoverable root, spec/ changes/ log/ present, every Cairn file carrying a valid `cairn:` type, each change having proposal.md and tasks.md, kebab-case ids, literal delta headings, and a log entry for every landed change. Everything else (prose, naming, ordering, extra files) is free.

## Everything else

For how this crate is built, where changes belong, and the Pimalaya standards it follows, read CONTRIBUTING.md and the src/lib.rs header (the crate architecture document).
