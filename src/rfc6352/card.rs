...
//!
//! Holds the [`CarddavCardRef`] and [`CarddavCardEntry`] types shared across the card
...
pub struct CarddavCardEntry {
    ...
    /// as the server returned it (see [`CarddavCardRef::id`]).
    pub id: String,
    /// Entity tag (RFC 9110 §8.8.3), without surrounding quotes.
    pub etag: Option<String>,
...
/// `CarddavCardEntry::id` / `CarddavCardRef::id`, used verbatim) into the card resource
