//! WebDAV XML response shapes (RFC 4918 §14).
//!
//! Generic over the `<prop>` payload so callers can plug their own
//! `serde::Deserialize` shape per coroutine.
...
/// Wrapper for arbitrary text bodies inside `<href>`, `<status>` or
/// any `xs:string` element.
#[derive(Clone, Debug, Deserialize)]
pub struct Value {
